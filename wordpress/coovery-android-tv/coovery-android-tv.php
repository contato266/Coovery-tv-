 ...
 * Description: Controla os carrosséis da Página inicial do app Coovery tv+ (Android TV e celular) via REST API.
 * Version: 1.1.0
 * Author: Coovery
...
define('COOVERY_ANDROID_TV_SCHEMA_VERSION', 2);
...
final class Coovery_Android_Tv_Plugin {
    ...
    public static function get_config(): array {
        ...
        if (isset($stored['slides']) && is_array($stored['slides']) && !isset($stored['tvSlides'])) {
            $stored['tvSlides'] = $stored['slides'];
            unset($stored['slides']);
        }

        if (!isset($stored['tvSlides']) || !is_array($stored['tvSlides'])) {
            $stored['tvSlides'] = self::default_slides();
        }
        if (!isset($stored['mobileSlides']) || !is_array($stored['mobileSlides'])) {
            $stored['mobileSlides'] = self::default_slides();
        }
    ...
    public static function default_config(): array {
        $defaults = self::default_slides();
        return [
            'schemaVersion' => COOVERY_ANDROID_TV_SCHEMA_VERSION,
            'tvSlides' => $defaults,
            'mobileSlides' => $defaults,
        ];
    }
}
...
register_activation_hook(__FILE__, static function (): void {
    if (get_option(COOVERY_ANDROID_TV_OPTION) === false) {
        update_option(COOVERY_ANDROID_TV_OPTION, Coovery_Android_Tv_Plugin::default_config(), false);
    }
});
